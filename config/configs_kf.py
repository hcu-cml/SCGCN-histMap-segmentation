...
class agriculture_configs(object):
    ...
    labels = land_classes
    nb_classes = 5
    palette = palette_land #palette_vsl
    weights = []
    ...
    input_size = [500, 500]
    scale_rate = 1.0/1.0
    val_size = [500, 500] #[1224, 1224]#[2248, 2248]
    train_samples = 1200 #540 - 900
    val_samples = 120 #60 - 90
    train_batch = 1 
    val_batch = 1
    ...
    def __init__(self, net_name=model, data=dataset, bands_list=bands, kf=1, k_folder=5, note=''):
        ...
        path_name = self.model + "_" + self.suffix_note
        check_mkdir(self.ckpt_path)
        check_mkdir(os.path.join(self.ckpt_path, path_name))
        ...
        check_mkdir(os.path.join(self.ckpt_path, path_name, subfolder))
        self.save_path = os.path.join(self.ckpt_path, path_name, subfolder)

        check_mkdir(os.path.join(self.ckpt_path, path_name))
        self.save_path = os.path.join(self.ckpt_path, path_name)
    ...
    def get_dataset(self):
        ...
        print("Number of training samples:" + str(len(train_dict)))
    ...
    def update_best_record(self, epoch, val_loss,
                           acc, acc_cls, mean_iu,
                           fwavacc, f1, prec, recall):
        print('----------------------------------------------------------------------------------------')
        print('[epoch %d]: [val loss %.5f], [acc %.5f], [acc_cls %.5f], [mean_iu %.5f], [fwavacc %.5f], [f1 %.5f], [prec %.5f], [recall %.5f]' % ( 
            epoch, val_loss, acc, acc_cls, mean_iu, fwavacc, f1 ,prec, recall))
        ...
        if mean_iu > self.best_record['mean_iu'] or f1 > self.best_record['f1']:
            ...
            self.best_record['prec'] = prec
            self.best_record['recall'] = recall
            return True
        else:
            return False
