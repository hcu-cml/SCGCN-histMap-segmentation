...
train_args.input_size = [500, 500]
train_args.scale_rate = 1.  # 256./512.  # 448.0/512.0 #1.0/1.0
train_args.val_size = [500, 500]
train_args.node_size = (64, 64)
#default batch size ist 9
train_args.train_batch = 9
train_args.val_batch = 9

train_args.lr = 0.00034641016151377546
train_args.weight_decay = 2e-5

train_args.lr_decay = 0.9
train_args.max_iter = 1000
...
def main():
    ...
    new_ep = 0
    while new_ep <= train_args.max_iter:
        starttime = time.time()
...
def update_ckpt(net, optimizer, epoch, new_ep, val_loss,
    ...
    acc, acc_cls, mean_iu, fwavacc, f1, prec, recall = evaluate(predictions_all, gts_all, train_args.nb_classes)
    ...
    updated = train_args.update_best_record(epoch, avg_loss, acc, acc_cls, mean_iu, fwavacc, f1, prec, recall)

    val_visual = []

    snapshot_name = 'epoch_%d_loss_%.5f_acc_%.5f_acc-cls_%.5f_mean-iu_%.5f_fwavacc_%.5f_f1_%.5f_lr_%.5f_precision_%.5f_recall_%.10f' % (
        epoch, avg_loss, acc, acc_cls, mean_iu, fwavacc, f1, prec, recall, optimizer.param_groups[0]['lr']
    )
